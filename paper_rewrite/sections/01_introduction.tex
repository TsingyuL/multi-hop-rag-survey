...
Many real-world questions cannot be answered by retrieving a single document. Their solutions require intermediate entities and hidden relations to be resolved before later sources become identifiable. Consider the HotpotQA question~\cite{yang2018hotpotqa}:
\begin{quote}
...
\begin{figure*}[tbp]
\centering
\resizebox{0.98\textwidth}{!}{\input{figures/fig1_functional_process}}
...
Existing surveys provide valuable views of multi-hop question answering, retrieval-augmented generation, reasoning, and agentic systems~\cite{mavi2022mhqasurvey,gao2023ragsurvey,fan2024ragllmsurvey,singh2025agenticragsurvey}. Their organizing axes usually emphasize model architecture, retrieval mechanism, reasoning form, data source, or agent framework. These views do not always make it easy to compare systems that use different architectures to solve the same information-acquisition function. They also tend to treat evaluation primarily at the answer or system level, even when a method's main claim concerns query generation, hop completion, stopping, source use, trajectory efficiency, or improvement across tasks. Table~\ref{tab:survey_positioning} summarizes the distinction at the level of survey families; it is intended to clarify emphasis rather than to claim that prior surveys entirely omit adjacent topics.

\begin{table*}[tbp]
\centering
\caption{Positioning relative to major survey families. ``Central'' indicates a principal organizing concern rather than an occasional topic.}
\label{tab:survey_positioning}
\small
\begin{tabularx}{\textwidth}{@{}p{2.2cm}p{2.55cm}Yp{3.35cm}p{3.15cm}@{}}
\toprule
Survey family & Primary unit & Typical organizing emphasis & Treatment of dependent search and learning & Evaluation emphasis \\
\midrule
Multi-hop QA surveys~\cite{mavi2022mhqasurvey} & QA task and model & Datasets, reasoning types, architectures, and supporting evidence & Later-query dependence is central within QA; persistent cross-task learning is usually outside scope & Answer and supporting-fact metrics \\
RAG surveys~\cite{gao2023ragsurvey,fan2024ragllmsurvey} & Retrieval--generation pipeline & Indexing, retrieval, augmentation, generation, and applications & Dependent retrieval is one advanced component; persistent learning is usually outside scope & Retrieval and generation quality \\
Agentic RAG surveys~\cite{singh2025agenticragsurvey} & Agent workflow & Planning, tools, memory, orchestration, and application patterns & Adaptive search is central; persistent memory or learning appears as a broader agent capability & End-to-end task and agent behavior \\
\textbf{This survey} & Dependent information acquisition & Initial retrieval, follow-up retrieval, evidence integration, and feedback learning & \textbf{Later-query dependence defines the scope; feedback learning is a dedicated category} & \textbf{Claim-aligned component, trajectory, grounding, cost, and transfer measures} \\
\bottomrule
\end{tabularx}
\end{table*}
