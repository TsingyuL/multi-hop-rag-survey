...
\begin{table*}[tbp]
\centering
\caption{Positioning relative to major survey families. ``Central'' indicates that the dimension is a principal organizing concern rather than an occasional topic.}
\label{tab:survey_positioning}
\footnotesize
\begin{tabularx}{\textwidth}{@{}p{2.0cm}p{2.35cm}Yp{2.05cm}p{1.85cm}p{2.75cm}@{}}
\toprule
Survey family & Primary unit & Typical organizing emphasis & Later-query dependence & Persistent learning & Evaluation emphasis \\
\midrule
Multi-hop QA surveys~\cite{mavi2022mhqasurvey} & QA task and model & Datasets, reasoning types, architectures, and supporting evidence & Central within QA & Usually outside scope & Answer and supporting-fact metrics \\
RAG surveys~\cite{gao2023ragsurvey,fan2024ragllmsurvey} & Retrieval--generation pipeline & Indexing, retrieval, augmentation, generation, and applications & One component among many & Usually outside scope & Retrieval and generation quality \\
Agentic RAG surveys~\cite{singh2025agenticragsurvey} & Agent workflow & Planning, tools, memory, orchestration, and application patterns & Central & Covered as an agent capability & End-to-end task and agent behavior \\
\textbf{This survey} & Dependent information acquisition & Initial retrieval, follow-up retrieval, evidence integration, and feedback learning & \textbf{Defining property} & \textbf{Dedicated category} & \textbf{Claim-aligned component, trajectory, grounding, cost, and transfer measures} \\
\bottomrule
\end{tabularx}
\end{table*}
